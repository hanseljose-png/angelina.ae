  ...
  return (
    <div style={{ background: 'var(--cream-dark)', position: 'relative', overflow: 'hidden', transition: 'transform 0.4s', transform: hovered ? 'translateY(-4px)' : 'none', cursor: 'pointer' }}
        ...
        <div style={{ aspectRatio: '3/4', position: 'relative', overflow: 'hidden', background: BG_COLORS[product.id] || (product.category === 'jewellery' ? BG_COLORS[2] : BG_COLORS[1]) }}>
          ...
          {hasImage && (
            <img src={product.image_url} alt={product.name}
              ...
              style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center top', transition: 'transform 0.6s ease', transform: hovered ? 'scale(1.05)' : 'scale(1)' }} />
          )}
      ...
      <div style={{ padding: '20px 20px 24px', background: 'var(--cream-dark)' }}>
        ...
        <Link to={`/shop/${product.id}`}>
          <div style={{ fontFamily: 'var(--font-serif)', fontSize: '19px', fontWeight: 400, color: 'var(--black)', marginBottom: '10px', lineHeight: 1.2 }}>{product.name}</div>
        </Link>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={() => addToCart(product)}
            style={{ fontSize: '9px', letterSpacing: '2px', textTransform: 'uppercase', color: 'var(--gold)', fontWeight: 600, border: 'none', background: 'none', cursor: 'pointer' }}>
            Add +
          </button>
        </div>
      </div>
