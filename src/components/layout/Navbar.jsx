...
export default function Navbar() {
  ...
  const isActive = (to) => {
    if (to === '/') return fullPath === '/'
    if (to === '/shop') return fullPath === '/shop'
    return fullPath === to || fullPath.includes(to)
  }
  ...
  return (
    <nav style={{
      ...
      <Link to="/" style={{ fontFamily: 'var(--font-serif)', fontSize: '28px', fontWeight: 300, letterSpacing: '8px', color: 'var(--gold)', textTransform: 'uppercase', textDecoration: 'none' }}>
        Angelina
      </Link>
      <ul style={{ display: 'flex', gap: '36px', listStyle: 'none' }}>
        {NAV_LINKS.map(({ label, to }) => (
          <li key={label}>
            <Link to={to} style={{ fontSize: '11px', letterSpacing: '3px', textTransform: 'uppercase', color: isActive(to) ? 'var(--gold)' : 'var(--cream)', fontWeight: 400, transition: 'color 0.3s', textDecoration: 'none' }}
              onMouseEnter={e => e.target.style.color = 'var(--gold)'}
              onMouseLeave={e => e.target.style.color = isActive(to) ? 'var(--gold)' : 'var(--cream)'}>
              {label}
            </Link>
          </li>
        ))}
