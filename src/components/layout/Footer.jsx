    ...
    <footer style={{ background: '#050505', padding: '60px 60px 30px', borderTop: '1px solid rgba(201,168,76,0.1)' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr', gap: '60px', marginBottom: '50px' }}>
        <div>
          ...
          <p style={{ fontSize: '12px', lineHeight: 1.8, color: 'rgba(250,248,243,0.35)', fontWeight: 300 }}>
            Luxury fashion and fine jewellery, crafted for the modern UAE woman. Based in Dubai, delivering elegance across the Emirates.
          </p>
        </div>
        {[
          { title: 'Collections', links: [['Shop All', '/shop'], ['Fashion', '/shop?cat=fashion'], ['Jewellery', '/shop?cat=jewellery'], ['New Arrivals', '/shop?cat=new']] },
          { title: 'Information', links: [['About Us', '/about'], ['Shipping & Returns', '/shipping'], ['Size Guide', '/size-guide'], ['Contact Us', '/contact']] },
        ].map(col => (
          <div key={col.title}>
            ...
            <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {col.links.map(([label, path]) => (
                <li key={label}><Link to={path} style={{ fontSize: '12px', color: 'rgba(250,248,243,0.4)', fontWeight: 300, transition: 'color 0.3s' }}
        ...
        <div>
          ...
          <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {['Dubai, UAE', 'hello@angelina.ae', '+971 50 000 0000'].map(t => (
              <li key={t} style={{ fontSize: '12px', color: 'rgba(250,248,243,0.4)', fontWeight: 300 }}>{t}</li>
            ))}
          </ul>
        </div>
      </div>
      <div style={{ borderTop: '1px solid rgba(201,168,76,0.08)', paddingTop: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          {['IG', 'TW', 'IN'].map(s => (
            <a key={s} href="#" style={{ width: '34px', height: '34px', border: '1px solid rgba(201,168,76,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', color: 'rgba(250,248,243,0.4)', letterSpacing: '1px', transition: 'all 0.3s' }}
