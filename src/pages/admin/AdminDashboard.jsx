    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#0d0d0d', fontFamily: 'var(--font-sans)' }}>
      <aside style={{ width: '240px', background: '#0a0a0a', borderRight: '1px solid rgba(201,168,76,0.1)', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        ...
        <div style={{ padding: '20px 28px', borderTop: '1px solid rgba(201,168,76,0.1)' }}>
          <Link to="/" target="_blank" style={{ display: 'block', fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', color: 'rgba(250,248,243,0.3)', marginBottom: '12px', textDecoration: 'none' }}>↗ View Store</Link>
          <button onClick={() => { logout(); window.location.href = '/admin/login' }}
            style={{ width: '100%', padding: '10px', border: '1px solid rgba(201,168,76,0.15)', background: 'transparent', color: 'rgba(250,248,243,0.3)', fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'var(--font-sans)' }}
      ...
      <main style={{ flex: 1, overflow: 'auto' }}>
        ...
        {tab === 'products' && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '1px', background: 'rgba(201,168,76,0.08)', borderBottom: '1px solid rgba(201,168,76,0.08)' }}>
            {stats.map(s => (
