  ...
  return (
    <main style={{ paddingTop: '100px', minHeight: '100vh', background: 'var(--cream)' }}>
      <div style={{ padding: '40px 60px' }}>
        <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: '52px', fontWeight: 300, color: 'var(--black)', marginBottom: '40px' }}>Shopping <em style={{ color: 'var(--gold)', fontStyle: 'italic' }}>Cart</em></h1>
              ...
              <div key={item.id} style={{ display: 'grid', gridTemplateColumns: '120px 1fr auto', gap: '24px', padding: '24px 0', borderBottom: '1px solid rgba(0,0,0,0.08)', alignItems: 'center' }}>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <button onClick={() => updateQty(item.id, item.qty - 1)} style={{ width: '28px', height: '28px', border: '1px solid rgba(0,0,0,0.2)', background: 'transparent', fontSize: '16px', cursor: 'pointer' }}>−</button>
                    <span style={{ fontSize: '14px', fontWeight: 500, minWidth: '20px', textAlign: 'center', color: 'var(--black)' }}>{item.qty}</span>
                    <button onClick={() => updateQty(item.id, item.qty + 1)} style={{ width: '28px', height: '28px', border: '1px solid rgba(0,0,0,0.2)', background: 'transparent', fontSize: '16px', cursor: 'pointer' }}>+</button>
                  </div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: '18px', fontWeight: 500, color: 'var(--black)', marginBottom: '12px' }}>AED {(item.price * item.qty).toLocaleString()}</div>
                  <button onClick={() => removeFromCart(item.id)} style={{ fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', color: 'var(--text-muted)', border: 'none', background: 'none', cursor: 'pointer' }}>Remove</button>
                </div>
              </div>
            ))}
          </div>
